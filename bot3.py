...
klines = np.array(client.get_historical_klines(symbolTicker, Client.KLINE_INTERVAL_1MINUTE, "12 hours ago UTC"))

#Volcar a un dataframe
datos = pd.DataFrame(klines.reshape(-1,12),dtype=float, columns=['open_time', 'open', 'high', 'low', 'close', 'volume', 'close_time', 'quote_asset_volume', 'number_of_trades', 'taker_buy_base_asset_volume', 'taker_buy_quote_asset_volume', 'ignore'])
...
data['upper_band'], data['middle_band'], data['lower_band'] = ta.BBANDS(data['Cierre'], timeperiod=20, nbdevup=2, nbdevdn=2, matype=0) #bandas de Bollinger

data['RSI'] = ta.RSI(data['Cierre'], timeperiod=14) # indicador RSI
data['STDDEV'] = ta.STDDEV(data['Cierre'], timeperiod=20) # desviacion estandar
...
plt.scatter(data.index, data['Compra'], label='Precio Compra', marker = '^', color = 'green')
plt.scatter(data.index, data['Venta'], label='Precio Venta', marker = 'v', color = 'red')
plt.scatter(data.index, data['DPO'], label='DPO', marker = '*', color = 'blue', alpha = 0.5)
